# Database Model
Describes the persistence model, schema conventions, and migration practices for an Aqevia world.

## Modular storage interface

- `aqevia-storage` defines the `StorageBackend` trait, `WorldRecord`, and `StorageController` that every implementation must follow. The Engine owns **when** to persist (flush scheduling and shutdown hooks), and each backend owns **how** (schema, migrations, queries).
...
- `aqevia-storage-sqlite` is the first backend. It stores records in `world_records` and keeps `schema_meta` for migration state.
- Schema:
  - `schema_meta(id INTEGER PRIMARY KEY, version INTEGER)` helps detect whether migrations ran.
...
- Migrations run via `init()` and are idempotent (`CREATE TABLE IF NOT EXISTS …`). Startup writes an initial `schema_meta` row (version `1`) when the table is empty, ensuring subsequent runs remain consistent.
