...
Rust formatting is enforced via `cargo fmt --all -- --check`, while Clippy (`cargo clippy --all-targets --all-features -- -D warnings`) catches lint regressions and policy violations. The preferred local/CI entry point for those commands (plus `cargo test --all`) is `./scripts/test.sh`, which prints clear banners, runs fmt, runs clippy, and then executes the full test suite in order.
